      ...
      <div class="space-y-6">
                  ...
                  ${this.dirHandle
                    ? html`
                        ...
                        >
                          ...
                          <span class="text-slate-400 font-mono">${activeT.filesLoaded(this.svgFilesCount)}</span>
                        </div>
                      `
                    ...
                    ? html`
                        <div class="p-3 bg-amber-bg border border-amber-border rounded-xl text-xs flex items-center justify-between shadow-inner">
                          <div class="flex items-center gap-2 text-amber-text font-medium">
                            ...
                            <span class="font-bold truncate max-w-50">${this.lang === "ko" ? "수동 로드된 임포트 셋" : "Manually Imported Set"}</span>
                          </div>
                          <span class="text-slate-400 font-mono">${activeT.filesDetected(this.svgFilesCount)}</span>
                        </div>
                      `
                    : html`
                        <div class="text-center py-2">
        ...
        <div class="glass-panel rounded-3xl p-6 shadow-xl relative overflow-hidden">
          ...
          <div class="space-y-4">
            ...
            <div>
              <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2.5">${activeT.imgFormat}</label>
              <div class="grid grid-cols-2 gap-3">
                <button
                  @click="${() => this.handleChangeFormat("png")}"
                  ?disabled="${this.isConverting}"
                  class="py-2.5 rounded-xl border text-xs font-bold transition-all font-sans cursor-pointer active:scale-95 ${this.exportFormat ===
                  "png"
            ...
            <div>
              <label class="block text-xs font-bold text-slate-400 uppercase tracking-widest mb-2.5">${activeT.scaleSetting}</label>
              <div class="space-y-3">
